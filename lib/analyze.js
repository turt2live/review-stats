...
module.exports = async function(prs, hasCommunity, orgName, repoName, teamName) {
    ...
    const metricsByWeek = { // "week" starts on the Sunday and is the day the first review request is made
        ...
        //     "reviewers": { username: # PRs for initial review }
        // }
    };
    const coreByWeek = {};
    ...
    for (const pr of prs) {
        ...
        if (!incomplete) {
            const reviewer = (firstReview['author'] || {})['login'] || "<unattributed>";
            if (!mainMetrics[key].reviewers[reviewer]) mainMetrics[key].reviewers[reviewer] = 0;
            if (!groupedByWeek[key].reviewers[reviewer]) groupedByWeek[key].reviewers[reviewer] = 0;

            mainMetrics[key].reviewers[reviewer]++;
            groupedByWeek[key].reviewers[reviewer]++;
        }
    }
